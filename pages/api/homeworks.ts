...
export default async function handler (req: NextApiRequest, res: NextApiResponse<Data>) {
  ...
  try {
    ...
    const homeworks = await db.select('*').from('homeworks').where({ roomcode: user.roomcode })
    if (req.method === 'GET') return res.send({ success: true, data: homeworks })

    if (!user.isTeacher) return res.send({ success: false, message: '선생님이 아닙니다!' })
    await db.insert(req.body).into('homeworks')
